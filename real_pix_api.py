...
class RealPixProvider:
    ...
    def __init__(self):
        ...
        pix_config_data = config.get_real_pix_config()
        
        self.api_key = pix_config_data['api_key']
        self.provider_url = pix_config_data['provider_url']
        self.merchant_id = pix_config_data['merchant_id']
        self.api_documentation = config.MEDIUS_PAG_API_DOCUMENTATION
        ...
        try:
            ...
            logger.error(f"Configuração PIX inválida: {e}")
            raise
    ...
    def _try_real_pix_provider(self, amount: float, customer_name: str, customer_cpf: str, customer_email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.provider_url:
                return {'success': False, 'error': 'REAL_PIX_PROVIDER_URL não configurada'}
